...
const addAddress = async (req: UserRequest, res: Response) => {
  try {
    ...
    res
      .status(201)
      .json(new ApiResponse(CONSTANTS.MESSAGES.ADDRESS_ADDED, addressResponse));
  } catch (error) {
    res
      .status(500)
      .json(
...
const getUserAddresses = async (req: UserRequest, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json(new ApiResponse(CONSTANTS.MESSAGES.ADDRESS_FOUND, addressResponse));
  } catch (error) {
    res
      .status(500)
      .json(
...
const getAddressById = async (req: UserRequest, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(CONSTANTS.MESSAGES.ADDRESS_FOUND, addressResponse!),
      );
  } catch (error) {
    res
      .status(500)
