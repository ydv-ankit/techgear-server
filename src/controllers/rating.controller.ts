...
const addRating = async (
  ...
  try {
    ...
    const [product, existingRating] = await Promise.all([
      ...
      prisma.rating.findFirst({ where: { product_id: product_id, user_id } }),
    ]);
    ...
    if (existingRating) {
      return res
        .status(400)
        .json(new ApiResponse(CONSTANTS.MESSAGES.RATING_ALREADY_EXISTS));
    }
    ...
    const updatedProductRating = product.rating
      ? product.rating + score / 2
      : score;
    ...
    res
      .status(201)
      .json(new ApiResponse(CONSTANTS.MESSAGES.RATING_ADDED, rating));
  } catch (error) {
    res
      .status(500)
      .json(
...
const getAllRatings = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json(new ApiResponse(CONSTANTS.MESSAGES.RATING_FOUND, ratings));
  } catch (error) {
    res
      .status(500)
      .json(
